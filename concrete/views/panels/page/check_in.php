    ...
    */?>
    ...
    <?php if (is_object($publishErrors) && $publishErrors->has()) { ?>
        ...
        <?php foreach($publishErrors->getList() as $error) { ?>
            <div class="text-danger"><strong><i class="fa fa-exclamation-circle"></i> <?php echo $error?></strong></div>
            <br/>
        <?php } ?>
        </div>
        ...
        if (is_object($pagetype)) { ?>
           ...
           <div class="text-info"><strong><i class="fa fa-question-circle"></i>
            <?php echo t('Fix common page errors from the <a href="#" data-launch-panel-detail="page-composer" data-panel-detail-url="%s" data-panel-transition="fade">Page Compose interface</a>.', URL::to('/ccm/system/panels/details/page/composer'))?>
            </strong></div>
            <br/>
            </div>
        <?php } ?>
